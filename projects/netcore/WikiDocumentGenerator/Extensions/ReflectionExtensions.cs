    ...
    public static class ReflectionExtensions
    {
        public static string MakeNodeName(this object obj)
        {
            StringBuilder GetDeclaringName(MemberInfo info, out Type[] classArgs)
            {
                ...
                {
                    dn.Append('`').Append(classArgs.Length);
                }
            ...
            switch (obj)
            {
                case MethodInfo mi:
                    ...
                    {
                        s.Append("``").Append(methodArgs.Length);
                    }
        ...
        public static IEnumerable<string> GetFullSignatureParts(this MemberInfo memberInfo, ICollection<MemberInfo> seeAlsoMembers)
		{
			...
			if (typeKind == TypeKind.Class || typeKind == TypeKind.Struct || typeKind == TypeKind.Interface)
			{
				...
				foreach (var baseInterface in baseInterfaces)
				{
					if (!(typeKind == TypeKind.Class && baseTypeInterfaces!.Contains(baseInterface)) &&
						!baseInterfaces.Any(x => /*XmlDocUtility.GetXmlDocRef(x) != XmlDocUtility.GetXmlDocRef(baseInterface) &&*/
		...
		private static bool IsKeyword(string? value) => s_keywords.Contains(value ?? string.Empty);
		...
		private static XmlDocVisibilityLevel Visibility = XmlDocVisibilityLevel.Protected;
		...
		private static XmlDocVisibilityLevel GetMostPublic(params XmlDocVisibilityLevel[] visibilityLevels) => (XmlDocVisibilityLevel) visibilityLevels.Max(x => (int) x);

		private static XmlDocVisibilityLevel GetMostPrivate(params XmlDocVisibilityLevel[] visibilityLevels) => (XmlDocVisibilityLevel) visibilityLevels.Min(x => (int) x);
